...
window.CodeMirror = (function() {
  ...
  CodeMirror.prototype = {
    ...
    // Locu-specific features.
    setFeatures: function(line, features) {
      if (typeof line == "number") {
        if (!window.menuTaskEditor.lines[line]) return null;
        line = getLine(this.doc, line);
      } else {
        if (indexOf(window.menuTaskEditor.lines, line) == -1) return null;
      }
    ...
    lineInfo: function(line) {
      ...
      return {line: n, handle: line, text: line.text, features: line.features, gutterMarkers: line.gutterMarkers,
  ...
  // Locu-specific: features separator.
  var SEP = "\t\t-\t\t";
  function handleFeatures(line) {
    if (line.text.indexOf("^") == 0 && line.text.indexOf(SEP)) {
      var split_text = line.text.split(SEP, 2);
      var features = split_text[0];
      line.text = split_text[1];

      line.features = JSON.parse(features.substring(1));
    } else {
      line.features = null;
    }
    return line;
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    getRange: function(from, to, lineSep, includeFeatures) {
      includeFeatures = includeFeatures || false;
      var lines = getBetween(this, clipPos(this, from), clipPos(this, to), includeFeatures);
      if (lineSep === false) return lines;
      return lines.join(lineSep || "\n");
    },
  ...
  function getBetween(doc, start, end, includeFeatures) {
    includeFeatures = includeFeatures || false;
    var out = [], n = start.line, text = "";
    doc.iter(start.line, end.line + 1, function(line) {
      if (includeFeatures && line.features && line.text.trim != "") {
        text = "^" + JSON.stringify(line.features) + SEP + line.text;
      } else {
        text = line.text;
      }
  ...
  function getLines(doc, from, to) {
    ...
    doc.iter(from, to, function(line) {
      if (line.features && line.text.trim != "") {
        out.push("^" + JSON.stringify(line.features) + SEP + line.text);
      } else {
        out.push(line.text);
      }
    });
